...
\paragraph{Schläger}
\mbox{}\\
\subparagraph{Bewegung des Schlägers} Bewegung kann Relativ oder Absulut erfolgen. Bei absultuter Bewegung wird der Schläger sehr stark springen und möglicherweise das Spiel zu einfach werden.
\subparagraph{Geschwindigkeit} Man sollte die Geschwindigkeit des Schläger begrenzen sodass das Spiel  schwieriger wird.
\subparagraph{Begrenzung} Der Schläger braucht 2 Begrenzungen damit er das Spielfeld nicht verlässt.
\subparagraph{Input der Controller} Der Input sollte so einfach wie möglich gestaltet werden. Am besten nur die Richtung und die Positionsänderung übertragen.
